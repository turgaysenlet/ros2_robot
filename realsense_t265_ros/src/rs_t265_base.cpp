...
#include <sstream>
#include "realsense/rs_t265_base.hpp"
...
{
  RCLCPP_INFO(node_.get_logger(), "RealSenseBase::startPipeline can_resolve...");
  RCLCPP_INFO(node_.get_logger(), "RealSenseBase::startPipeline can_resolve can_resolve: %d.", (int)cfg_.can_resolve(pipeline_));
  RCLCPP_INFO(node_.get_logger(), "RealSenseBase::startPipeline can_resolve.");

  RCLCPP_INFO(node_.get_logger(), "RealSenseBase::startPipeline resolve...");
...
{
  RCLCPP_INFO(node_.get_logger(), "RealSenseBase::setupStream %s %d...", STREAM_NAME.at(stream.first).c_str(), stream.second);
  ...
  if (stream == ACCEL || stream == GYRO) {
    ...
    if (enable == true) {
      enable_[stream] = true;
      RCLCPP_INFO(node_.get_logger(), "RealSenseBase::setupStream enable_stream %s %d...", STREAM_NAME.at(stream.first).c_str(), stream.second);
    ...
    if (enable == true) {
      enable_[stream] = true;
      RCLCPP_INFO(node_.get_logger(), "RealSenseBase::setupStream enable_stream %s %d - %dx%d %d %d...", STREAM_NAME.at(stream.first).c_str(), stream.second, info.width, info.height, (int)STREAM_FORMAT.at(stream.first), info.fps);
...
{
  // RCLCPP_INFO(node_.get_logger(), "RealSenseBase::publishImageTopic %d...", frame.as<rs2::video_frame>().get_profile().stream_index());
